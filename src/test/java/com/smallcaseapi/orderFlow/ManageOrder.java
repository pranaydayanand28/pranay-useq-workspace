...
import com.smallcaseapi.payload.ManagePayload;
import io.restassured.response.Response;

public class ManageOrder extends BaseTest {
	...
	private static final Logger logger = LoggerFactory.getLogger(ManageOrder.class.getName());

	@Test(priority = 7, testName = "To validate Manage order API", description = "Manage order API : to validate Manage order API upon valid set of inputs")
	@Parameters({"broker"})
	public static void manageOrder(String broker) throws FileNotFoundException, IOException, InterruptedException {

		logger.info("Creating request for Manage order API - start");

		response = given()
				.filter(new RequestLoggingFilter(captor))
				.spec(RequestSpec.requestSpecification(broker))
				.body(ManagePayload.managePayload(broker))
				.when()
				.post(SmallcaseResource.placeOrder)
				.then()
				.extract().response();

		logger.info("Creating request for Manage order API - end");
