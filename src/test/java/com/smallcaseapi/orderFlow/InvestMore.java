...
import com.smallcaseapi.payload.InvestMorePayload;
...
public class InvestMore extends BaseTest {
    ...
    private static final Logger logger = LoggerFactory.getLogger(InvestMore.class.getName());

    @Test(priority = 5, testName = "To validate Invest more API", description = "Invest More API : to validate Invest more API upon valid set of inputs")
    @Parameters({"broker"})
    public static void investMore(String broker) throws IOException, InterruptedException {

        logger.info("Creating request for invest more order API - start");

        response = given()
                .filter(new RequestLoggingFilter(captor))
                .spec(RequestSpec.requestSpecification(broker))
                .body(InvestMorePayload.investMore(broker))
                .when()
                .post(SmallcaseResource.placeOrder)
                .then()
                .extract().response();

        logger.info("Creating request for invest more order API - end");
